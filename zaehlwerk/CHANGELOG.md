...
- **[Kritisch] Vollständige Aussperrung nach Einspielen einer HA-Sicherung in eine Standalone-Instanz:** Aus einem Home-Assistant-Add-on übernommene Konten haben `password_hash = None` (dort meldet der Supervisor an, es gibt kein lokales Passwort). Wurde eine solche `.gz`-Sicherung in eine eigenständig (ohne HA) laufende Instanz eingespielt, ersetzte der Restore die komplette Datenbank – inklusive des zuvor lokal angelegten Admin-Kontos – durch die passwortlosen HA-Konten. `setup_required()` prüfte nur `user_count == 0`, sah die vorhandenen HA-Konten und bot deshalb **keine** Ersteinrichtung an, während sich zugleich niemand anmelden konnte: dauerhafter Lockout. **Fix:** neue Prüfung `auth.local_login_available()` (mindestens ein aktives Konto mit lokalem Passwort). `setup_required()` gilt im Standalone-Betrieb jetzt auch dann, wenn zwar Konten existieren, aber keines lokal anmeldbar ist. Der `/api/auth/setup`-Endpunkt lässt in diesem Wiederherstellungsfall die Einrichtung eines Administrator-Kontos zu und **übernimmt** ein bereits vorhandenes, gleichnamiges Konto (Passwort setzen, aktivieren, zum Administrator machen), statt es zu duplizieren. Die Oberfläche erklärt den Fall auf dem Einrichtungs-Screen gesondert (`recovery`-Flag in `/api/auth/status`). Regressionstests: `tests/test_auth_recovery.py`.
